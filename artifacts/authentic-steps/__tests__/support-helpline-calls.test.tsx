...
// ─── 'this week' practical-ideas tip keyword assertions ───────────────────────

/**
 * Regression guard: confirms that navigating via urgency='this week' to the
 * 'practical-ideas' support type yields a tip whose title contains "try" and
 * whose body contains "grounding" for every one of the five area IDs.
 * A copy-paste bug in area-keyed tip lookups for this urgency path would
 * return blank or wrong copy without triggering the coarser length-only checks
 * that already exist in the 'this week' urgency routing suite.
 */
describe("Support screen – 'this week' practical-ideas tip keyword assertions", () => {
  let root: ReturnType<typeof create>;
  let openURLSpy: jest.SpyInstance;
  ...
  it.each(ALL_AREAS)(
    "urgency='this week' → area=\"$label\" → practical-ideas: tip box present, title contains \"try\", body contains \"grounding\", no call button",
    async ({ areaId }) => {
      // Step 1 — open triage
      await act(async () => {
        root.root.findByProps({ testID: 'triage-start-btn' }).props.onPress();
...
// ─── 'today' someone-to-listen tip keyword assertions ─────────────────────────

/**
 * Regression guard: confirms that navigating via urgency='today' to the
 * 'someone-to-listen' support type yields a tip whose title contains "listen"
 * and whose body contains "trusted" for every one of the five area IDs.
 * A copy-paste bug or per-area conditional that swaps or blanks the tip for
 * any area on the 'today' path would go undetected with the coarser
 * length-only checks that already existed for this urgency path.
 */
describe("Support screen – 'today' someone-to-listen tip keyword assertions", () => {
  let root: ReturnType<typeof create>;
  let openURLSpy: jest.SpyInstance;
  ...
  it.each(ALL_AREAS)(
    "urgency='today' → area=\"$label\" → someone-to-listen: tip box present, title contains \"listen\", body contains \"trusted\", no call button",
    async ({ areaId }) => {
      // Step 1 — open triage
      await act(async () => {
        root.root.findByProps({ testID: 'triage-start-btn' }).props.onPress();
...
// ─── 'today' practical-ideas tip keyword assertions ───────────────────────────
 ...
 * whose body contains "grounding" for every one of the five area IDs.
 * A copy-paste bug or per-area conditional that swaps or blanks the tip for
 * any area on the 'today' → practical-ideas path would go undetected with the
 * coarser length-only checks that already existed for this urgency path.
 */
describe("Support screen – 'today' practical-ideas tip keyword assertions", () => {
  let root: ReturnType<typeof create>;
  let openURLSpy: jest.SpyInstance;
  ...
  it.each(ALL_AREAS)(
    "urgency='today' → area=\"$label\" → practical-ideas: tip box present, title contains \"try\", body contains \"grounding\", no call button",
    async ({ areaId }) => {
      // Step 1 — open triage
      await act(async () => {
        root.root.findByProps({ testID: 'triage-start-btn' }).props.onPress();
